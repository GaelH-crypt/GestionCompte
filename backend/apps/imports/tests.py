...
class CategorizerTest(TestCase):
    ...
    def test_energy(self):
        self.assertEqual(suggest_category('PRELEVEMENT EDF PARTICULIERS'), 'Logement')
    ...
    def test_restaurant(self):
        self.assertEqual(suggest_category('RESTAURANT LE BISTROT'), 'Restaurants & Bars')

    def test_fast_food(self):
        self.assertEqual(suggest_category('CB MCDONALD PARIS'), 'Restaurants & Bars')

    def test_streaming_subscription(self):
        self.assertEqual(suggest_category('PRLV NETFLIX.COM'), 'Factures & Abonnements')

    def test_shopping(self):
        self.assertEqual(suggest_category('CB AMAZON EU'), 'Shopping')

    def test_insurance(self):
        self.assertEqual(suggest_category('PRLV ASSURANCE MAIF'), 'Assurances')

    def test_paiement_not_misread_as_income(self):
        # 'PAIE'/'CAF' ne doivent pas matcher 'PAIEMENT'/'CAFE'.
        self.assertEqual(suggest_category('PAIEMENT CARREFOUR'), 'Alimentation')
        self.assertEqual(suggest_category('CB CAFE DE LA GARE'), 'Restaurants & Bars')

    def test_property_tax_before_housing(self):
        # 'TAXE FONCIERE' doit être classé en Impôts & Taxes, pas Logement
        # (le mot-clé FONCIER de Logement ne doit pas le capturer en premier).
        self.assertEqual(suggest_category('PRLV TAXE FONCIERE'), 'Impôts & Taxes')

    def test_assurance_vie_before_insurance(self):
        # 'ASSURANCE VIE' doit aller en Épargne & Investissements, pas Assurances.
        self.assertEqual(
            suggest_category('ASSURANCE VIE BOURSORAMA'), 'Épargne & Investissements'
        )
